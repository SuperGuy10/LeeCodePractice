...
class Solution(object):
    def countBinarySubstrings(self, s):
        ...
        count = 0
        for char in s:
            if char == init: #to check if the current value is as same as previous one
                count += 1
            else:
                res.append(count)
                init = char #update new initial
                count = 1
        res.append(count)# at the end of for loop, it won't update the last count to res.
        sum = 0
